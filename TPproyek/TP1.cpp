...
void insertAfter(int pos, string new_data) 
{
	if(head == NULL) { 
...
void update_data(string old, string newd){
   int pos = 0;
...
main(){
	...
	readdata();
	menu:
	cout<<"Jumlah Node : "<<getCount()<<endl;
	printList();
	cout<<endl;
	menus();cin>>pil;
	if(pil == 1){
		...
		cout<<endl;
		cout<<"Masukkan posisi [Index] setelah yang akan diinput : ";cin>>index;
		cout<<"Masukkan data yang akan diinput : ";cin>>input;
		insertAfter(index,input);
		system("cls");
		goto menu;
	}
