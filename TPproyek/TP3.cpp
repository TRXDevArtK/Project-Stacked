...
class tampil{
	public:
		void read();
		void test(string c4,string gc4,string kc4,string from,string hc4);
		int getCount(int pil2);
		void printList();
};

int tampil::getCount(int pil2) 
{ 
...
void tampil::test(string c4,string gc4,string kc4,string from,string hc4){
...
void tampil::read(){
	string number,jb,gb,kb,ku,hb,from;
	ifstream ior("data1.txt",ios::in);
	while(ior>>number>>jb>>gb>>kb>>ku){
		//cout<<number<<" "<<jb<<" "<<gb<<" "<<kb<<" "<<ku<<endl;
		test(number,jb,gb,kb,ku);
	}
}
